...
import SubloteMap from "../../cultivo/sublote/widgets/SubloteMap";
import { useCreateSublote } from "../../cultivo/sublote/hooks/useCreateSublote";
import { useUpdateSublote } from "../../cultivo/sublote/hooks/useUpdateSublote";
import type { CreateSubloteDTO, Sublote } from "../../cultivo/sublote/model/types";
import { useGeoData } from "../hooks/useGeoData";
import { useLoteList } from "../../cultivo/lote/hooks/useLoteList";
...
export default function SubloteModal({ isOpen, onClose, subloteToEdit }: Props) {
  ...
  const { lotes } = useLoteList(); // Need lotes for selection
  ...
  useEffect(() => {
    if (isOpen) {
        if (subloteToEdit) {
            ...
            setLoteSeleccionado(String(subloteToEdit.lote?.id_lote_pk || ""));
  ...
  const handleSubmit = async () => {
    ...
    if (!loteSeleccionado) {
      setMapMensaje("Debes seleccionar un lote.");
      return;
    }

    if (coordenadas.length < 3) {
      setMapMensaje("Debes dibujar un polígono válido (mínimo 3 puntos).");
      return;
    }
    if (area <= 0) {
      setMapMensaje("El área debe ser mayor a 0");
      return;
    }
    setMapMensaje("");
    ...
    try {
      ...
      if (error.response?.data) {
        ...
        setMapMensaje(Array.isArray(errorMsg) ? errorMsg.join(", ") : errorMsg);
      } else {
        setMapMensaje("No se pudo guardar el sublote. Verifica que esté completamente dentro del lote y no se solape con otros sublotes.");
      }
    }
  };
