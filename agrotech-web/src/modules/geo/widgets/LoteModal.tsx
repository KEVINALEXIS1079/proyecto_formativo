...
import LoteMap from "../../cultivo/lote/widgets/LoteMap";
import { useCreateLote } from "../../cultivo/lote/hooks/useCreateLote";
import { useUpdateLote } from "../../cultivo/lote/hooks/useUpdateLote";
import type { CreateLoteDTO, Lote } from "../../cultivo/lote/model/types";
...
export default function LoteModal({ isOpen, onClose, loteToEdit }: Props) {
  const { data: allLotes = [], refetch } = useGeoData();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (loteToEdit) {
        await loteService.updateLote(loteToEdit.id_lote_pk, newLote);
      } else {
        await loteService.createLote(newLote);
      }
      refetch();
      onClose();
    } catch (error: any) {
      ...
      if (error.response) {
          ...
          setErrorNombre(`Error: ${JSON.stringify(error.response.data.message || "Error al guardar")}`);
      }
    }
  };
      ...
      <ModalContent>
        ...
        <ModalBody>
          <div className="grid grid-cols-12 gap-6 h-full">
            ...
            <div className="col-span-8 h-full min-h-[400px] bg-gray-100 dark:bg-zinc-900 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-800 relative">
                <LoteMap 
                    coordenadas={coordenadas.map(c => ({ latitud: c.latitud_lote, longitud: c.longitud_lote }))} 
                    setCoordenadas={(newCoords) => setCoordenadas(newCoords.map(c => ({ latitud_lote: c.latitud, longitud_lote: c.longitud })))} 
                    setArea={setArea}
                    isEditing={!isReadOnly}
                    lotesExistentes={allLotes
                        .filter(l => l.id_lote_pk !== loteToEdit?.id_lote_pk)
                        .map(l => ({
                            nombre: l.nombre_lote,
                            coordenadas: l.coordenadas_lote.map(c => ({ latitud: c.latitud_lote, longitud: c.longitud_lote }))
                        }))
                    }
                />
            </div>
        ...
        <ModalFooter>
          ...
          {isReadOnly ? (
            ...
            <Button className="text-white" color="success" onPress={handleSubmit} isLoading={isLoading}>
                Guardar Cambios
            </Button>
          )}
        </ModalFooter>
