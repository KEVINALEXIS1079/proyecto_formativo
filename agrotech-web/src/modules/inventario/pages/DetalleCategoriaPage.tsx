...
export default function DetalleCategoriaPage() {
  ...
  const categoriaId = id ? parseInt(id) : 0;

  const { data: categoria, isLoading, error } = useCategoriaById(categoriaId);
  const removeMutation = useRemoveCategoria();
  const { data: insumosData } = useInsumoList({ categoriaId });
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8 px-4 md:px-6">
        <div className="mx-auto max-w-7xl">
          <div className="bg-white rounded-2xl shadow-xl p-12 border border-gray-200 text-center">
            ...
            <p className="text-gray-600 text-lg">No se pudo cargar la información de la categoría. Por favor, intenta nuevamente.</p>
          </div>
        </div>
      </div>
    );
  ...
  const handleConfirmDelete = async () => {
    try {
      await removeMutation.mutateAsync(categoria.id);
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-8 border border-gray-200">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h1 className="text-4xl font-bold text-gray-900 mb-2">Detalle de Categoría</h1>
                <p className="text-gray-600 text-lg">Información completa de la categoría seleccionada</p>
              </div>
            </div>
        ...
        <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-xl p-8 border border-gray-200 hover:shadow-2xl transition-shadow duration-300 mb-8">
          ...
          <div className="space-y-6">
            <div className="bg-white/50 rounded-xl p-4 border border-gray-100">
              ...
              <p className="mt-2 text-xl font-medium text-gray-900">{categoria.nombre}</p>
            </div>
            <div className="bg-white/50 rounded-xl p-4 border border-gray-100">
              <label className="block text-sm font-semibold text-gray-700 uppercase tracking-wide">Descripción</label>
              <p className="mt-2 text-gray-700 leading-relaxed">{categoria.descripcion || "Sin descripción"}</p>
            </div>
          </div>
        </div>
