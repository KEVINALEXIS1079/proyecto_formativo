...
export default function DetalleProveedorPage() {
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8 px-4 md:px-6">
        <div className="mx-auto max-w-7xl">
          <div className="bg-white rounded-2xl shadow-xl p-12 border border-gray-200 text-center">
            ...
            <p className="text-gray-600 text-lg">No se pudo cargar la información del proveedor. Por favor, intenta nuevamente.</p>
          </div>
        </div>
      </div>
    );
  ...
  const handleEdit = () => {
    navigate(`/inventario/proveedores/${proveedor.id}/editar`);
  };
  ...
  const handleConfirmDelete = async () => {
    try {
      await removeMutation.mutateAsync(proveedor.id);
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-8 border border-gray-200">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h1 className="text-4xl font-bold text-gray-900 mb-2">Detalle de Proveedor</h1>
                <p className="text-gray-600 text-lg">Información completa del proveedor seleccionado</p>
              </div>
            </div>
        ...
        <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-xl p-8 border border-gray-200 hover:shadow-2xl transition-shadow duration-300 mb-8">
          ...
          <div className="space-y-6">
            <div className="bg-white/50 rounded-xl p-4 border border-gray-100">
              ...
              <p className="mt-2 text-xl font-medium text-gray-900">{proveedor.nombre}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-xl p-8 border border-gray-200 hover:shadow-2xl transition-shadow duration-300">
          <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-3">
            ...
            Insumos de este Proveedor
          </h2>
          <Table aria-label="Tabla de insumos" removeWrapper isVirtualized={false}>
            <TableHeader columns={columns}>
              {(column) => <TableColumn key={column.key}>{column.label}</TableColumn>}
            </TableHeader>
            <TableBody items={insumos} emptyContent="No hay insumos de este proveedor">
              {(item: any) => (
