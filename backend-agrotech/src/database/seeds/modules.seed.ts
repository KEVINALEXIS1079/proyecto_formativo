...
export async function seedModules(dataSource: DataSource) {
  ...
  const actividadResponsableRepo =
    dataSource.getRepository(ActividadResponsable);
  ...
  const movimientoProduccionRepo =
    dataSource.getRepository(MovimientoProduccion);
  ...
  const adminUser = await usuarioRepo.findOne({
    where: { correo: process.env.ADMIN_EMAIL || 'agrotechsena2025@gmail.com' },
  });
  ...
  const usuariosData = [
    ...
    },
    {
      ...
      identificacion: '1110554345',
      correo: 'ana.martinez@sena.edu.co',
      passwordHash: await bcrypt.hash('password123', 10),
      rolId: 3, // Aprendiz
      estado: 'activo',
      emailVerifiedAt: new Date('2024-02-12'),
    },
    {
      nombre: 'Pedro Luis',
      apellido: 'Ramírez Gómez',
      identificacion: '1110565678',
      correo: 'pedro.ramirez@sena.edu.co',
      passwordHash: await bcrypt.hash('password123', 10),
      rolId: 2, // Instructor
      estado: 'activo',
    ...
    },
    {
      ...
      identificacion: '1110587234',
      correo: 'diego.vargas@sena.edu.co',
      passwordHash: await bcrypt.hash('password123', 10),
      rolId: 4, // Pasante
      estado: 'activo',
    ...
    },
    {
      ...
      apellido: 'Rojas Díaz',
      identificacion: '1110598567',
      correo: 'camila.rojas@sena.edu.co',
      passwordHash: await bcrypt.hash('password123', 10),
      rolId: 3, // Aprendiz
      estado: 'activo',
      emailVerifiedAt: new Date('2024-02-18'),
    },
  ];
  ...
  for (const loteData of lotesData) {
    const existing = await loteRepo.findOne({
      where: { nombre: loteData.nombre },
    });
    if (!existing) {
  ...
  const sublotesData = [
    {
      nombre: 'Esperanza Sector A - Tomates',
      loteId: loteIds[0],
      areaM2: 3000,
      areaHa: 0.3,
  ...
  for (const subloteData of sublotesData) {
    const existing = await subLoteRepo.findOne({
      where: { nombre: subloteData.nombre },
    });
    if (!existing) {
  ...
  const cultivosData = [
    {
      ...
      descripcion:
        'Tomate cherry variedad Sweet Million, producción para mercado local',
      subLoteId: subloteIds[0],
      fechaSiembra: new Date('2024-01-15'),
      estado: 'activo',
    },
    {
      nombreCultivo: 'Tomate Chonto',
      tipoCultivo: 'Hortaliza',
      descripcion: 'Tomate chonto para procesamiento industrial',
      subLoteId: subloteIds[0],
      fechaSiembra: new Date('2024-02-01'),
      estado: 'activo',
    },
    {
      nombreCultivo: 'Lechuga Crespa',
      tipoCultivo: 'Hortaliza',
      descripcion: 'Lechuga crespa verde, ciclo de 45 días',
      subLoteId: subloteIds[1],
      fechaSiembra: new Date('2024-03-10'),
      estado: 'activo',
    },
    {
      nombreCultivo: 'Lechuga Romana',
      tipoCultivo: 'Hortaliza',
      descripcion: 'Lechuga romana para ensaladas gourmet',
      subLoteId: subloteIds[1],
      fechaSiembra: new Date('2024-03-20'),
      estado: 'activo',
    },
    {
      nombreCultivo: 'Cilantro',
      tipoCultivo: 'Aromática',
      descripcion: 'Cilantro para condimento, siembra escalonada',
      subLoteId: subloteIds[2],
      fechaSiembra: new Date('2024-02-15'),
      estado: 'activo',
    },
    {
      nombreCultivo: 'Albahaca',
      tipoCultivo: 'Aromática',
      descripcion: 'Albahaca dulce italiana',
      subLoteId: subloteIds[2],
      fechaSiembra: new Date('2024-02-20'),
      estado: 'activo',
    },
    {
      nombreCultivo: 'Maíz Dulce',
      tipoCultivo: 'Cereal',
      descripcion: 'Maíz dulce híbrido para consumo fresco',
      subLoteId: subloteIds[3],
      fechaSiembra: new Date('2024-01-20'),
      estado: 'activo',
    },
    {
      nombreCultivo: 'Papa Criolla',
      tipoCultivo: 'Tubérculo',
      descripcion: 'Papa criolla para mercado regional',
      subLoteId: subloteIds[4],
      fechaSiembra: new Date('2024-02-05'),
      estado: 'activo',
    },
    {
      nombreCultivo: 'Papa Pastusa',
      tipoCultivo: 'Tubérculo',
      descripcion: 'Papa pastusa para industria',
      subLoteId: subloteIds[4],
      fechaSiembra: new Date('2024-02-10'),
      estado: 'activo',
    },
    {
      nombreCultivo: 'Fresa',
      tipoCultivo: 'Frutal',
      descripcion: 'Fresa variedad Albión en sistema de producción elevado',
  ...
  for (const cultivoData of cultivosData) {
    const existing = await cultivoRepo.findOne({
      where: { nombreCultivo: cultivoData.nombreCultivo },
    });
    if (!existing) {
  ...
  for (const catData of categoriasData) {
    const existing = await categoriaRepo.findOne({
  ...
  const proveedoresData = [
    {
      nombre: 'AgroQuímicos S.A.',
      contacto: 'contacto@agroquimicos.com',
      telefono: '3001234567',
    },
    {
      nombre: 'Semillas del Valle',
      contacto: 'ventas@semillasvalle.com',
      telefono: '3019876543',
    },
  ];
  ...
  for (const provData of proveedoresData) {
    const existing = await proveedorRepo.findOne({
      where: { nombre: provData.nombre },
    });
    if (!existing) {
  ...
  for (const almData of almacenesData) {
    const existing = await almacenRepo.findOne({
      where: { nombre: almData.nombre },
    });
    if (!existing) {
  ...
  const insumosData = [
    {
      ...
      unidadUso: 'g',
      tipoMateria: 'Solido',
      factorConversionUso: 25000,
      stockPresentacion: 2,
      stockUso: 50000,
      precioUnitarioPresentacion: 375000,
      precioUnitarioUso: 15,
    ...
    },
    {
      ...
      unidadUso: 'cm3',
      tipoMateria: 'Liquido',
      factorConversionUso: 20000,
      stockPresentacion: 1,
      stockUso: 20000,
      precioUnitarioPresentacion: 500000,
      precioUnitarioUso: 25,
  ...
  for (const insumoData of insumosData) {
    const existing = await insumoRepo.findOne({
  ...
  for (const actData of actividadesData) {
    const existing = await actividadRepo.findOne({
      where: { nombre: actData.nombre, cultivoId: actData.cultivoId },
    });
    if (!existing) {
  ...
  const responsablesData = [
    {
      ...
      usuarioId: instructor?.id || adminId,
      rol: 'Instructor',
      horas: 8,
      precioHora: 5000,
      costo: 40000,
  ...
  for (const respData of responsablesData) {
    const existing = await actividadResponsableRepo.findOne({
      where: {
        actividadId: respData.actividadId,
        usuarioId: respData.usuarioId,
      },
    });
    if (!existing) {
  ...
  const serviciosData = [
    {
      actividadId: actividadIds[0],
      nombreServicio: 'Preparación del suelo',
      horas: 4,
      precioHora: 5000,
      costo: 20000,
    },
    {
      actividadId: actividadIds[0],
      nombreServicio: 'Siembra de semillas',
      horas: 6,
      precioHora: 5000,
      costo: 30000,
    },
    {
      actividadId: actividadIds[1],
      nombreServicio: 'Aplicación de fertilizante',
      horas: 6,
      precioHora: 5000,
      costo: 30000,
    },
  ...
  for (const servData of serviciosData) {
    const existing = await actividadServicioRepo.findOne({
      where: {
        actividadId: servData.actividadId,
  ...
  for (const actInsData of actividadInsumosData) {
    const existing = await actividadInsumoRepo.findOne({
      where: {
        actividadId: actInsData.actividadId,
        insumoId: actInsData.insumoId,
      },
    });
    if (!existing) {
  ...
  if (actividadInsumosCreados > 0) {
    console.log(
      `    ${actividadInsumosCreados} insumos asignados a actividades`,
    );
  }
  ...
  const evidenciasData = [
    {
      actividadId: actividadIds[0],
      imagenes: ['https://example.com/foto-siembra.jpg'],
      descripcion: 'Foto de la siembra',
    },
    {
      actividadId: actividadIds[1],
      imagenes: ['https://example.com/video-fertilizante.mp4'],
      descripcion: 'Video de aplicación de fertilizante',
    },
  ];
  ...
  for (const eviData of evidenciasData) {
    const existing = await actividadEvidenciaRepo.findOne({
      where: {
        actividadId: eviData.actividadId,
        descripcion: eviData.descripcion,
      },
    });
    if (!existing) {
  ...
  const tiposSensorData = [
    ...
    },
    {
      nombre: 'Humedad',
      unidad: '%',
      descripcion: 'Sensor de humedad relativa',
    },
    {
      nombre: 'pH del suelo',
      unidad: 'pH',
      descripcion: 'Sensor de pH del suelo',
    },
  ];
  ...
  for (const tipoData of tiposSensorData) {
    const existing = await tipoSensorRepo.findOne({
  ...
  const sensoresData = [
    {
      nombre: 'Sensor Temp Norte A',
      tipoSensorId: tipoSensorIds[0],
      cultivoId: cultivoIds[0],
      protocolo: 'MQTT',
      ubicacion: 'Centro del sublote',
      estado: 'activo',
    },
    {
      nombre: 'Sensor Humedad Norte A',
  ...
  for (const sensorData of sensoresData) {
    const existing = await sensorRepo.findOne({
      where: { nombre: sensorData.nombre },
    });
    if (!existing) {
  ...
  const productosData = [
    {
      ...
      descripcion: 'Tomate cherry fresco',
      unidadBase: 'kg',
    },
    {
      nombre: 'Maíz dulce',
      descripcion: 'Maíz dulce en mazorca',
      unidadBase: 'kg',
    },
    {
      nombre: 'Papa blanca',
      descripcion: 'Papa blanca de calidad',
      unidadBase: 'kg',
    },
  ];
  ...
  for (const prodData of productosData) {
    const existing = await productoAgroRepo.findOne({
  ...
  for (const loteProdData of lotesProdData) {
    const existing = await loteProduccionRepo.findOne({
      where: {
        productoAgroId: loteProdData.productoAgroId,
        cultivoId: loteProdData.cultivoId,
      },
    });
    if (!existing) {
  ...
  const usuarioActivo = await usuarioRepo.findOne({
    where: { estado: 'activo' },
  });
  ...
  for (const movProdData of movimientosProdData) {
    const existing = await movimientoProduccionRepo.findOne({
      where: {
        loteProduccionId: movProdData.loteProduccionId,
        tipo: movProdData.tipo,
      },
    });
    if (!existing) {
  ...
  if (movimientosProdCreados > 0) {
    console.log(
      `    ${movimientosProdCreados} movimientos de producción creados`,
    );
  }
  ...
  const clientesData = [
    {
      ...
      telefono: '3024567890',
      email: 'compras@supermercadoxyz.com',
      direccion: 'Calle 123 #45-67',
    },
    {
      nombre: 'Restaurante El Sabor',
      identificacion: '800987654',
      telefono: '3035678901',
  ...
  for (const cliData of clientesData) {
    const existing = await clienteRepo.findOne({
      where: { nombre: cliData.nombre },
    });
    if (!existing) {
  ...
  for (const detData of detallesVentaData) {
    const existing = await ventaDetalleRepo.findOne({
      where: {
        ventaId: detData.ventaId,
        loteProduccionId: detData.loteProduccionId,
      },
    });
    if (!existing) {
  ...
  const facturasData = [
    {
      ...
      numero: 'F001-2024',
      prefijo: 'F001',
      fechaEmision: new Date('2024-06-20'),
      vencimiento: new Date('2024-07-20'),
    },
    {
      ventaId: ventaIds[1],
      numero: 'F002-2024',
      ...
      vencimiento: new Date('2024-08-05'),
    },
  ];

  let facturasCreadas = 0;
  for (const factData of facturasData) {
    const existing = await facturaRepo.findOne({
      where: { ventaId: factData.ventaId },
    });
    if (!existing) {
  ...
  const tiposCultivoWikiData = [
    {
      nombre: 'Hortalizas',
      descripcion: 'Cultivos de hortalizas como tomate, lechuga, etc.',
    },
    {
      nombre: 'Cereales',
      descripcion: 'Cultivos de cereales como maíz, trigo, etc.',
    },
    {
      nombre: 'Tubérculos',
      descripcion: 'Cultivos de tubérculos como papa, yuca, etc.',
    },
  ];
  ...
  for (const tipoWikiData of tiposCultivoWikiData) {
    const existing = await tipoCultivoWikiRepo.findOne({
      where: { nombre: tipoWikiData.nombre },
    });
    if (!existing) {
  ...
  const epasData = [
    {
      ...
      tipoEpa: 'buenas_practicas',
      descripcion: 'Proceso para preparar el suelo antes de la siembra',
      instrucciones: 'Arar, fertilizar, nivelar el terreno.',
      creadoPorUsuarioId: usuarioActivo?.id ?? 1,
    },
    {
      nombre: 'Siembra',
      tipoEpa: 'buenas_practicas',
      descripcion: 'Técnicas de siembra adecuadas',
      instrucciones: 'Profundidad, distancia entre semillas, riego inicial.',
      creadoPorUsuarioId: usuarioActivo?.id ?? 1,
    },
    {
      nombre: 'Manejo de plagas',
      tipoEpa: 'manejo_plagas',
      descripcion: 'Control integrado de plagas',
      instrucciones: 'Identificación, métodos preventivos y curativos.',
      creadoPorUsuarioId: usuarioActivo?.id ?? 1,
    },
  ];
  ...
  for (const epaData of epasData) {
    const existing = await ePARepo.findOne({
      where: { nombre: epaData.nombre },
    });
    if (!existing) {
  ...
  for (const relData of relacionesData) {
    const existing = await ePATipoCultivoWikiRepo.findOne({
      where: {
        epaId: relData.epaId,
        tipoCultivoWikiId: relData.tipoCultivoWikiId,
      },
    });
    if (!existing) {
  ...
  if (relacionesCreadas > 0) {
    console.log(
      `    ${relacionesCreadas} relaciones EPA - Tipo Cultivo creadas`,
    );
  }
